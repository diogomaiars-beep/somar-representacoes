...
function renderCats(){
  document.getElementById('cats').innerHTML=categories.map(x=>
    `<button class="cat ${x[0]===active?'active':''}" onclick='selectCat(${js(x[0])})'>${x[1]} ${esc(x[0])}</button>`
  ).join('')
}
...
function selectCat(x){
  ...
  renderCats();
  render();
  document.getElementById('title').textContent=x==='Todos'?'Catálogos':x;
}

function render(){
  const q=document.getElementById('search').value.toLowerCase().trim();
  const list=catalogs.filter(c=>
    (active==='Todos'||c.category===active) &&
  ...
  document.getElementById('grid').innerHTML=list.length
    ? list.map(c=>{
      const cover=c.cover_url
        ? `<img src="${esc(c.cover_url)}" alt="Capa de ${esc(c.name)}" loading="lazy">`
        : `<div class="mark">${esc((c.brand||'S')[0].toUpperCase())}</div>`;

      return `
        <article class="card">
          <button class="card-main" onclick='openCatalog(${js(c.id)})' aria-label="Abrir ${esc(c.name)}">
            <div class="cover">
              ${cover}
              <span class="tag">${esc(c.category)}</span>
            </div>
            <div class="body">
              <div class="brand-name">${esc(c.brand||'Somar Representações')}</div>
              <h3>${esc(c.name)}</h3>
...
function openCatalog(id){
  ...
  const cover=c.cover_url
    ? `<img src="${esc(c.cover_url)}" alt="Capa de ${esc(c.name)}">`
    : `<div class="detail-mark">${esc((c.brand||'S')[0].toUpperCase())}</div>`;
    ...
    <div class="modal-backdrop" onclick="closeCatalog(event)">
      <div class="modal" onclick="event.stopPropagation()">
        ...
        <div class="detail-content">
          ...
          <div class="detail-brand">${esc(c.brand||'Somar Representações')}</div>
          <h2>${esc(c.name)}</h2>
          <p>${esc(c.description||'Consulte o catálogo de produtos da Somar Representações.')}</p>
          <div class="detail-actions">
            <a class="btn primary big" href="${url}" target="_blank" rel="noopener">👁️ Ver catálogo</a>
            <a class="btn big" href="${url}${url.includes('?')?'&':'?'}download=true">⬇️ Baixar PDF</a>
            <button class="btn big" onclick='shareCatalogById(${js(c.id)})'>📲 Compartilhar</button>
          </div>
        </div>
      </div>
    </div>`;
...
async function shareCatalogById(id){
  ...
  const url=c.pdf_url||'';
  const text=`Olá! Segue o catálogo ${c.name} da Somar Representações.${url?' '+url:''}`;

  if(navigator.share){
    try{
      await navigator.share({title:c.name,text,url:url||location.href});
      return;
    }catch(e){}
  }
...
async function loadCatalogs(){
  ...
  if(error){
    console.error(error);
    document.getElementById('status').textContent='Não foi possível carregar os catálogos agora.';
    return;
  }
